...
from database.schema_metadata import  (
    user_table_name, lounge_table_name, tx_table_name, country_table_name, event_table_name, amenity_table_name, airport_table_name, order_table_name,
                               user_columns, lounge_columns, tx_columns, country_columns, event_columns, amenity_columns, airport_columns, order_columns,
                               user_types, lounge_types, tx_types,country_types, event_types, amenity_types, airport_types, order_types
...
def db_initializer():
    ...
    create_database_schema(
        ...
        columns = columns,
        # foreign_keys = {tx_columns['OrderID']: f'Order({order_columns["OrderID"]})'}
    )
    # ----------------------------------------
    columns= {}
    for i in order_columns:
        columns[order_columns[i]] = order_types[order_columns[i]]
    create_database_schema(
        table_name = order_table_name,
        columns = columns,

        foreign_keys = {order_columns['UserID']: f'User({user_columns["UserID"]})', 
                    order_columns['LoungeID']: f'Item({lounge_columns["LoungeID"]})'}
    )
    # ----------------------------------------
    columns= {}
